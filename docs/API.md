...
- `POST /api/runs {preset:PresetBody, sourceArtifactId?, budgetUsd?:number|null, select?:boolean}` → `RunView` (400 + `issues` when the chain is invalid — before any paid call). Omitting sourceArtifactId uses the session's accepted source. Passing the artifact id of any earlier output = "New run from this artifact".
  **Auto-bridge** (`autoBridge`, default `true`): when the source kind differs from the first step's input and one step can convert it (image→text or text→image), that step is prepended with the fastest tested model and `definition.auto: true`. It is an ordinary, visible step; nothing else in the pipeline changes. Pass `autoBridge:false` to get the 400 instead.
  **Instruction set** (`instructionSet?: 'faithful'|'forensic'|'minimal'|'storyteller'|'childlike'`, from `INSTRUCTION_SETS` in `shared/types.ts`): replaces the static instruction of EVERY step (bridge included) for this run only; types, models, params and the saved preset are untouched. The run is named `<pipeline> [<set name>]`. Unknown id → 400.
  **Interactive** (`interactive:true`): the run may be created with `steps: []`; creating it makes no provider call.
- `POST /api/runs/:id/steps {type: StepType, twist?: string (≤500), modelId?: string, instructionSet?: string}` → `RunView`. Interactive runs only (400 otherwise). Appends ONE step (fastest tested model for the type unless `modelId` is given; `twist` is appended to the static instruction as "Additional direction: …") and runs exactly that step. 400 if the type does not accept the run's latest output (a video ends the path); 409 while the run is running/failed/stopped. Existing steps are never modified. To branch, create a new interactive run with `sourceArtifactId` set to any earlier output.
